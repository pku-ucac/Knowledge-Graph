...
**习题1** 如果初始时刻$t=0$系统静止在平衡位置 $(x=\dot{x}=0)$ ，试求系统在下列几种形式的外力 $F(t)$ 作用下的强迫振动。
...
**习题2** 设直到 $t=0$ 时系统静止在平衡位置，力 $F$ 的变化规律为：当 $t<0$ 时 $F=0$ ，当 $0<t<T$ 时 $F=F_0 t / T$ ，当 $t>T$ 时 $F=F_0$ (图24)。试求在该力作用后系统振动的最后振幅。
...
**习题3** 同习题2，力 $F_0$ 是常数，只在有限时间间隔 $T$ 内作用（图25）。
...
**习题4** 同习题2，但力在从零到 $T$ 时间间隔内按规律 $F = F_0 t / T$ 作用（图26）。

**解**：用同样方法可求得：

$$a = \frac{F_0}{T m \omega^3} \sqrt{\omega^2 T^2 - 2 \omega T \sin \omega T + 2(1 - \cos \omega T)}$$

**习题5** 同习题2，但力在从零到 $T = 2 \pi / \omega$ 时间间隔内按规律 $F = F_0 \sin \omega t$ 作用（图27）。
